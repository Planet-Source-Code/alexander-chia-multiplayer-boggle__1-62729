...
'
' LOG:
' - 2003.12.12: · Added List() prop.
'               · Added <index> param. in AddItem
'               · Fixed Refresh (added listbox refresh)
' - 2003.12.15: · DrawText 'string lenght' param. has
'                 been changed from Len(<string>) to -1.
'                 Extended char-sets now supported.
'                 Thanks to CodeClub.
' - 2003.12.17: · See pvCalcPixelsPerDlgUnit()
'                 Thanks Vlad.
' - 2003.12.22: · Fixed 'Property Get List()'
...
Private Sub UserControl_Resize()
  ...
  Dim nIdx As Integer
    ...
    '-- Headers height
    m_lHeaderHeight = 1.1 * UserControl.TextHeight("")
    
    '-- Update headers height
    For nIdx = 1 To UBound(m_tCols)
        m_tCols(nIdx).rHeader.y2 = m_lHeaderHeight
        m_tCols(nIdx).rHeaderCaption.y2 = m_lHeaderHeight
    Next nIdx
    
    '-- Set headers background and pad rects.
    Call SetRect(m_rHeaders, 0, 0, ScaleWidth, m_lHeaderHeight)
    Call SetRect(m_rHeadersPad, m_tCols(UBound(m_tCols)).rHeader.x2, 0, ScaleWidth, m_lHeaderHeight)
    
    '-- Resize list box
    Call lstReport.Move(0, m_lHeaderHeight, ScaleWidth, ScaleHeight - m_lHeaderHeight)
End Sub
...
Private Sub UserControl_Paint()
  ...
  Dim nIdx As Integer
    ...
    '-- Erase background
    Call FillRect(hDC, m_rHeaders, GetSysColorBrush(COLOR_BTNFACE))
    
    '-- Draw headers
    For nIdx = 1 To UBound(m_tCols)
        With m_tCols(nIdx)
            Call DrawEdge(hDC, .rHeader, BDR_RAISEDOUTER, BF_BOTTOM + BF_RIGHT)
            Call DrawText(hDC, .sHeaderCaption, -1, .rHeaderCaption, .eColumnAlignment + DT_SINGLELINE + DT_VCENTER)
        End With
    Next nIdx
    '-- Draw headers pad
    Call DrawEdge(hDC, m_rHeadersPad, BDR_RAISEDOUTER, BF_BOTTOM + BF_RIGHT)
End Sub
...
Public Sub AddHeader(ByVal Width As Long, ByVal Alignment As eAlignmentCts, ByVal Caption As String)
  ...
  Dim snPixelsPerDlgUnit As Single
    ...
    nCols = UBound(m_tCols)
    
    '-- Increase count
    ReDim Preserve m_tCols(0 To nCols + 1)
    ReDim Preserve m_lTabs(0 To nCols + 1)
    
    '-- Define header/column
    With m_tCols(nCols + 1)
        Call SetRect(.rHeader, m_tCols(nCols).rHeader.x2, 0, m_tCols(nCols).rHeader.x2 + Width, m_lHeaderHeight)
    ...
    For nIdx = 1 To UBound(m_tCols)
        With m_tCols(nIdx)
            Select Case .eColumnAlignment
                Case LeftJustify:  m_lTabs(nIdx) = m_tCols(nIdx).rHeaderCaption.x1 / snPixelsPerDlgUnit
                Case RightJustify: m_lTabs(nIdx) = -(m_tCols(nIdx).rHeaderCaption.x2 + 1) / snPixelsPerDlgUnit
            End Select
        End With
    Next nIdx
    Call SendMessage(lstReport.hWnd, LB_SETTABSTOPS, UBound(m_tCols), m_lTabs(1))
...
Public Property Get HeadersCount() As Integer
    HeadersCount = UBound(m_tCols)
End Property
  ...
  Const sChars   As String = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890"
    ...
    If (hDC) Then
        ...
        If (GetTextExtentPoint32(hDC, sChars, Len(sChars), sz)) Then
          ...
          '*pvCalcPixelsPerDlgUnit = (2 * cxAvLBChar) / cxDlgBase
